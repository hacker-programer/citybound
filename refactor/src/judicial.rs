...
impl JudicialSystem {
    ...
    pub fn tick(&mut self, city_treasury: &mut f64, dt: f32) -> Vec<JudicialEvent> {
        ...
        // Recolectar primero para evitar conflicto de borrow
        let mut troll_suits: Vec<(String, f64)> = Vec::with_capacity(8);
        for firm in &self.law_firms {
            if firm.is_patent_troll && rng_pool::rng_fast() < 0.05 * dt {
                troll_suits.push((firm.name.clone(), firm.annual_revenue * 0.1));
            }
        }
        for (firm_name, amount) in troll_suits {

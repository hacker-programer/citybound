// Módulo de Quadtree Espacial
//
// TÉCNICA COMÚN #7: Árboles de Colisión Estáticos (Quadtree)
...
// dinámicamente durante la simulación pagando O(log N) por operación.
//
// TÉCNICA AVANZADA #14: BVH balanceados
// El quadtree se construye con balanceo estático para garantizar
// búsquedas O(log N) estrictas.
//
// Memoria: ~4 * MAX_DEPTH * capacidad nodos ≈ 64 KB

/// Profundidad máxima del quadtree (4 niveles = 256 celdas hoja)
const MAX_DEPTH: u8 = 6;
/// Máximo de entidades por nodo antes de subdividir
const MAX_ENTITIES_PER_NODE: usize = 16;
/// Capacidad máxima de entidades en todo el quadtree
const MAX_TOTAL_ENTITIES: usize = 16384;

/// Índice de entidad en el quadtree
...
/// Nodo del quadtree (almacenado en un array para localidad de caché)
#[derive(Copy, Clone, Debug)]
struct QuadNode {
    /// AABB de este nodo
    bounds: AABB,
    /// Índice del primer hijo en la lista de nodos (0 si es hoja)
    first_child: u32,
    /// Cantidad de hijos (0 o 4)
    child_count: u8,
    /// Profundidad actual
    depth: u8,
    /// Índice del primer entity_id en entity_data (0 si vacío)
    first_entity: u32,
...
/// Quadtree espacial con almacenamiento contiguo [TA#9: alineación 64B]
#[repr(align(64))]
pub struct Quadtree {
    ...
    entity_count: usize,
    /// Raíz del quadtree (índice en nodes)
    root_idx: u32,
}

impl Quadtree {
    /// Crea un quadtree vacío con capacidad pre-reservada [TC#2]
    pub fn new(world_w: f32, world_h: f32) -> Self {
        // [TC#2]: Pre-reservar capacidad máxima
    ...
    pub fn insert(&mut self, bounds: AABB) -> QuadEntity {
        ...
        // Insertar en el árbol recursivamente
    ...
    pub fn query_radius<F: FnMut(QuadEntity)>(
        ...
        // Expandir bounds: crear AABB que contenga el círculo
    ...
    /// Número total de entidades en el quadtree
    #[inline(always)]
            ...
            for handle in entities {
                ...
                for i in 0..4 {
                    ...
                    if self.nodes[ci].bounds.contains_point(ent_bounds.x, ent_bounds.y) {
                        // Insertar en este hijo (directamente, sin recursión extra)
    ...
    fn remove_from_all(&mut self, handle: QuadEntity) {
        // Búsqueda lineal por simplicidad (el quadtree es para acelerar
        // consultas espaciales, no eliminaciones)
        let mut pos = 0;
        while pos < self.entity_ids.len() {
            if self.entity_ids[pos] == handle {
                // Encontrar qué nodo contiene esta posición
...
mod tests {
    ...
    fn test_quadtree_insert_query() {
        ...
        // Consultar región (0,0) a (30,30)
        let query = AABB::new(0.0, 0.0, 30.0, 30.0);
        let mut found = Vec::new();
        qt.query(&query, &mut |e| found.push(e));

        assert!(found.contains(&h1), "h1 debe estar en región (0,0)-(30,30)");
        assert!(!found.contains(&h2), "h2 no debe estar en esa región");
        assert!(!found.contains(&h3), "h3 no debe estar en esa región");
    }
    ...
    fn test_quadtree_update() {
        ...
        assert!(found2.contains(&h), "Entidad debe estar en nueva posición");
    }
    ...
    fn test_quadtree_clear() {
        ...
        let mut count = 0;
        qt.query(&query, &mut |_| count += 1);
        assert_eq!(count, 0, "Quadtree vacío no debe retornar entidades");
    }
